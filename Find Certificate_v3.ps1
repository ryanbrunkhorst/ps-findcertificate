...
if (!$results) {echo $computer | out-file $PSScriptRoot"\results\"$CurrentDate"\ComputersMissingCertorWinRMError_"$CurrentDate.txt -Append} #if the cert is missing or WinRM fails
    else {echo $computer | out-file $PSScriptRoot"\results\"$CurrentDate"\ComputersWithCert_"$CurrentDate.txt -Append} #if the cert is found add to this txt file

}

else{ #if test-connection fails, add computer name to offline text file
echo $computer" offline"
echo $computer | out-file $PSScriptRoot"\results\"$CurrentDate"\Offline_"$CurrentDate.txt -Append
}
